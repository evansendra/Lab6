...
public class MarketAnalyzer implements Runnable {
	 ...
	 * 
	 * @param text
	 *            This is the text of the error message that is to be pri8nted.
	 */
	 ...
	 *            when things happen.
	 */
	public MarketAnalyzer(String[] symbols, StockQuoteGeneratorInterface wi,
			StockTickerAudioInterface audioPlayer) {
		this.audioPlayer = audioPlayer;
		for (int index = 0; index < symbols.length; index++) {
			try {
				StockQuoteAnalyzer analyzer = new StockQuoteAnalyzer(
						symbols[index], wi.createNewInstance(symbols[index]),
						audioPlayer);
	...
	public void updateData() {
		...
		while (iter.hasNext() == true) {
			...
			try {
				instance.refresh();
				out.println(instance.getDisplayMessage());
	...
	public static void main(String[] args) {
		if (args.length < 2) {
			...
			MarketAnalyzer ma = new MarketAnalyzer(truncArgs,
					new YahooStockQuoteHTMLQuoteExtractor(), new AudioErrorPlayer());
